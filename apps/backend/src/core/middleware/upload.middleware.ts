...
// Separate small-size, spreadsheet-only instance for catalog bulk import — distinct
// from the media ALLOWED_MIME_TYPES set used for archive/document uploads. CSV is
// parsed with a small hand-rolled parser (core/utils/csv.ts) rather than an npm CSV
// library, since the actively-maintained options ship unpatched high-severity CVEs.
// XLSX uses `exceljs` (actively maintained; only a moderate, low-reachability
// transitive advisory via `uuid`, unlike `xlsx`'s directly-exploitable parser CVEs).
...
export const uploadCatalogImport = multer({
  ...
  fileFilter: (_req, file, cb) => {
    if (CATALOG_IMPORT_MIME_TYPES.has(file.mimetype) || /\.(csv|xlsx)$/i.test(file.originalname)) {
      cb(null, true);
    } else {
      cb(new Error(`Unsupported file type for catalog import: ${file.mimetype}. Please upload a .csv or .xlsx file.`));
    }
  },
}).single("file");
