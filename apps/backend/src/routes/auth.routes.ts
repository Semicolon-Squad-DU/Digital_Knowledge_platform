...
router.patch(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const values: unknown[] = [req.user!.user_id];
    ...
    if (bio !== undefined) {
      updates.push({ field: "bio", value: bio });
      values.push(bio);
    }
    if (department !== undefined) {
      updates.push({ field: "department", value: department ?? null });
      values.push(department ?? null);
    }
    if (avatar_url !== undefined) {
      updates.push({ field: "avatar_url", value: avatar_url });
      values.push(avatar_url);
    }

    if (updates.length === 0) {
      const user = await queryOne(
        `SELECT user_id, name, email, role, department, bio, avatar_url, membership_status, created_at, updated_at
         FROM users WHERE user_id = $1`,
        [req.user!.user_id]
      );
      res.json({ success: true, data: user });
    ...
    const user = await queryOne(
      `UPDATE users
       ...
       WHERE user_id = $1
       RETURNING user_id, name, email, role, department, bio, avatar_url, membership_status, created_at, updated_at`,
      values
    );

    logger.info("User profile updated", { user_id: req.user!.user_id });
    res.json({ success: true, data: user });
  })
);
...
router.patch(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    logger.info("User password changed", { user_id: req.user!.user_id });
    res.json({ success: true, data: updatedUser, message: "Password changed successfully" });
  })
);
