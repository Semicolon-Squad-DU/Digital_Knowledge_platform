...
// PATCH /api/research/:id — Metadata edit (researcher who owns it or admin)
router.patch(
  "/:id",
  authenticate,
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const output = await queryOne<{ output_id: string; uploaded_by: string }>(
      "SELECT output_id, uploaded_by FROM research_outputs WHERE output_id = $1",
      [req.params.id]
    );
    if (!output) throw new AppError(404, "Research output not found");

    const role = req.user!.role;
    if (role !== "admin" && output.uploaded_by !== req.user!.user_id) {
      throw new AppError(403, "You can only edit your own research outputs");
    }

    const {
      title, abstract, authors, keywords, doi, output_type,
      lab_id, published_date, journal_name, volume, issue, pages,
    ...
    const updated = await queryOne(
      `UPDATE research_outputs SET
         ...
         lab_id         = COALESCE($7,  lab_id),
         published_date = COALESCE($8,  published_date),
         journal_name   = COALESCE($9,  journal_name),
         volume         = COALESCE($10, volume),
         issue          = COALESCE($11, issue),
      ...
      [
        ...
        keywords    ? (Array.isArray(keywords)  ? keywords  : JSON.parse(keywords as string)) : null,
        doi         !== undefined ? doi         : null,
        output_type || null,
        lab_id      !== undefined ? lab_id      : null,
        published_date !== undefined ? published_date : null,
        journal_name   !== undefined ? journal_name   : null,
