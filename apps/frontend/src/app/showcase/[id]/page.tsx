...
function DownloadReportButton({ projectId }: { projectId: string }) {
  ...
  return (
    <button
      ...
      disabled={loading}
      className="inline-flex items-center gap-1.5 hover:underline text-sm disabled:opacity-60"
...
export default function ShowcaseDetailPage() {
  ...
  if (isLoading) {
    return (
      <CollectionShell active="Showcase">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 text-sm" style={{ fontFamily: SANS, color: C.body }}>
          Loading project...
        </div>
      </CollectionShell>
    );
  ...
  return (
    <CollectionShell active="Showcase">
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8" style={{ fontFamily: SANS }}>
      ...
      <div className="gh-box p-6">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
          <div>
            <h1 className="text-2xl font-bold" style={{ fontFamily: SERIF, color: C.ink }}>{project.title}</h1>
            <p className="text-sm mt-1" style={{ color: C.body }}>{project.department} · {project.semester}</p>
          </div>
        ...
        <p className="text-sm mt-4" style={{ color: C.ink }}>{project.abstract}</p>

        <div className="mt-4 text-sm space-y-1" style={{ color: C.ink }}>
        ...
        {!!project.team_members?.length && (
          <div className="mt-4">
            <p className="font-medium text-sm mb-1" style={{ color: C.ink }}>Team Members</p>
            <ul className="list-disc list-inside text-sm" style={{ color: C.body }}>
        ...
        <div className="mt-5 flex items-center gap-3 text-sm">
          {isAuthenticated && project.source_code_url && (
            <a href={project.source_code_url} target="_blank" rel="noopener noreferrer" style={{ color: C.accent }} className="hover:underline">
              Source Code
            </a>
          )}
          ...
          {!isAuthenticated && (project.source_code_url || project.report_url) && (
            <span className="text-xs italic" style={{ color: C.body }}>
              Please <Link href={`/login?redirect=/showcase/${projectId}`} className="hover:underline font-semibold" style={{ color: C.accent }}>sign in</Link> to view source code or report.
            </span>
          )}
          {canReviewProject && (project.status === "pending_review" || project.status === "changes_requested") && (
            <Link href={`/showcase/review/${project.project_id}`} style={{ color: C.accent }} className="hover:underline">
              Review Page
            </Link>
          )}
        </div>
        ...
        {isAuthenticated && project.video_url && (
          <div>
            <p className="font-medium text-sm mt-5 mb-1" style={{ color: C.ink }}>Demo Video</p>
        ...
        {isAuthenticated && project.report_url && (
          <div>
            <p className="font-medium text-sm mt-5 mb-1" style={{ color: C.ink }}>Project Report</p>
